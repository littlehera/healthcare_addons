...
def execute(filters=None):
	...
	for row in rows:
		...
		row['company'] = get_company(row['sales_invoice'])
...
def get_company(si):
	rows = frappe.db.sql("""SELECT company from `tabInvoice Payment Table` where parent = %s and payment_mode = 'Charge to HMO'""",si)
